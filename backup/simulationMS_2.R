...
vaQuit = 0 # action value for quit across all states
# holdOnSteps = round(2 / (tMax / nTimeStep))
holdOnSteps = 0
# learning model 
...
for(tIdx in 1 : nTrial) {
  for(t in 1 : nTimeStep){
    ...
    if(action == 'quit' || rewardOccur || t == nTimeStep){
      ...
      timeWaited[tIdx] = ifelse(rewardOccur,  NA, mean(timeTicks[ t + 1]))
      break
    }
  }  
}

simulationResults = data.frame(trialEarnings, timeWaited, scheduledWait = thisTrialData$scheduledWait)

# 
kmGrid = seq(0, tMax, by=0.1) # grid on which to average survival curves.
kms = kmsc(simulationResults,tMax,'simulation data',T,kmGrid)
kms = kmsc(thisTrialData,tMax,'experimental data',T,kmGrid)
